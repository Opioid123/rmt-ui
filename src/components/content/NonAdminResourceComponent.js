...
class NonAdminResourceComponent extends React.Component {
    ...
    save() {debugger;
        ...
        if(this.state.dialogType === 'add')
            this.props.dispatch(addResource(this.props.ldapResource));
        else
    ...
    onResourceSelect(e){
        ...
        //this.props.dispatch(selectLdap({...e.data, role: ''}));
    }
    ...
    checkResource(resource){
        ...
        if (this.checkEmpty(resource.role)){
            this.error = true;
            this.props.dispatch(addError('Please select role'));
        }
    }
    ...
    render() {
        ...
        var header = <div style={{'textAlign':'left'}}>
                        ...
                        <Button style={{float:'right',backgroundColor: '#F86402', color: 'white', fontWeight: 'bold'}} variant="contained" onClick={this.addNew} color="#F86402">Add Resource</Button>
                    </div>;
        ...
        (<div className="ui-dialog-buttonpane p-clearfix">
            ...
            <Button style={{backgroundColor: '#F86402', color: 'white', fontWeight: 'bold'}}  onClick={() => {this.setState({displayDialog: false})}}>Cancel</Button>
        </div>):
        (<div className="ui-dialog-buttonpane p-clearfix">
            <Button style={{backgroundColor: '#F86402', color: 'white', fontWeight: 'bold'}} variant="contained" onClick={this.save}>Save</Button>
            <Button style={{backgroundColor: '#F86402', color: 'white', fontWeight: 'bold', marginRight:10, marginLeft:10}} variant="contained" onClick={this.delete}>Delete</Button>
            ...
            <div>
                ...
                <Dialog open={this.state.displayDialog}>
                    ...
                    <DialogContent dividers>
                        ...
                        {
                            this.state.dialogType !== 'add' ? <TextField disabled  margin="dense" id="ldap" label="LDAP" defaultValue={this.state.resource.ldap} variant="outlined" fullWidth
                            ...
                            <Autocomplete 
                                ...
                                renderInput={params => (
                                <TextField {...params} required  label="LDAP"  margin="dense" variant="outlined" fullWidth InputProps={{ ...params.InputProps, type: 'search' }} />
                                )}
                            />
                            </div>
                        }
                        ...
                        <FormControl variant="outlined" style={{marginTop: 8, minWidth: 120, }}>
                            {
                                this.state.dialogType !== 'add' ? <InputLabel margin="dense"  id="labelRole">Role</InputLabel>:
                                <InputLabel required margin="dense"  id="labelRole">Role</InputLabel>
                            }
                            <Select labelId="lblRole" id="role" value={this.props.ldapResource.role} 
                            onChange={(e) => {this.updateProperty('role', e.target)}} displayEmpty margin="dense" width="100%">
...
export default connect(state => getResourceListing(state))(NonAdminResourceComponent);
